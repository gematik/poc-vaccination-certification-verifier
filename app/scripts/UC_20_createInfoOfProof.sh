...
# Description (short): Performs use case UC_20_Create_InformationOfProof.
#        For more information see
#        a. ../README.sh and
#        b. CmdLine.ACTION_INFOPROOF_CREATE
# Usage: ./UC_20_createInfoOfProof prefix ...
...
if [ -f "${APP}" ] && [ -x "${APP}" ]; then
  # echo "app present"
  "${APP}" $ACTION "${prefix}" "${name}" "${dayOfBirth}" "${expirationDate}" $*
else
  echo "app absent"
fi # end else
